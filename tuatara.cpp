...
public:
   ...
   */
  std::vector<std::string> decode(const torch::Tensor &token_dists,
                                  bool raw = false) {
    ...
    for (int64_t i = 0; i < token_dists.size(0); ++i) {
      ...
      std::vector<size_t> id_vector(ids.data_ptr<int64_t>(),
...
private:
  ...
  std::tuple<torch::Tensor, torch::Tensor> filter(const torch::Tensor &probs,
...
std::pair<std::vector<cv::RotatedRect>, cv::Mat>
get_detected_boxes(torch::Tensor textmap, torch::Tensor linkmap,
                   float text_threshold, float link_threshold, float low_text,
                   bool estimate_num_chars = false) {
  torch::Tensor textmap_normalized =
      (textmap - textmap.min()) / (textmap.max() - textmap.min());
  torch::Tensor linkmap_normalized =
      (linkmap - linkmap.min()) / (linkmap.max() - linkmap.min());

  cv::Mat textmap_cv(textmap_normalized.size(0), textmap_normalized.size(1),
                     CV_32F, textmap_normalized.data_ptr<float>());
  cv::Mat linkmap_cv(linkmap_normalized.size(0), linkmap_normalized.size(1),
                     CV_32F, linkmap_normalized.data_ptr<float>());
  ...
  cv::Mat labels, stats;
  cv::Mat centroids;
  int nLabels = cv::connectedComponentsWithStats(text_score_comb, labels, stats,
                                                 centroids, 4);
  ...
  for (int k = 1; k < nLabels; ++k) {
    ...
    int niter =
        static_cast<int>(std::sqrt(size * std::min(w, h) / (w * h) * 2));
    ...
    cv::Mat kernel = cv::getStructuringElement(cv::MORPH_RECT,
                                               cv::Size(1 + niter, 1 + niter));
    cv::dilate(segmap(cv::Rect(sx, sy, ex - sx, ey - sy)),
               segmap(cv::Rect(sx, sy, ex - sx, ey - sy)), kernel);
    ...
    if (std::abs(1 - box_ratio) <= 0.1) {
      ...
      cv::Point2f new_box[4] = {cv::Point2f(l, t), cv::Point2f(r, t),
                                cv::Point2f(r, b), cv::Point2f(l, b)};
      std::copy(new_box, new_box + 4, box);
    }

    int startidx = std::distance(
        box, std::min_element(box, box + 4,
...
std::tuple<cv::Mat, float, cv::Size> resize_aspect_ratio(const cv::Mat &img,
                                                         int square_size,
                                                         int interpolation,
                                                         float mag_ratio = 1) {
  int height = img.rows;
  ...
  int target_h32 =
      target_h % 32 != 0 ? target_h + (32 - target_h % 32) : target_h;
  int target_w32 =
      target_w % 32 != 0 ? target_w + (32 - target_w % 32) : target_w;
...
adjust_result_coordinates(const std::vector<cv::RotatedRect> &polys,
                          float ratio_w, float ratio_h, float ratio_net = 2) {
  ...
  for (const auto &poly : polys) {
    ...
    cv::RotatedRect adjusted_rect =
        cv::minAreaRect(std::vector<cv::Point2f>(corners, corners + 4));
...
std::vector<std::vector<std::pair<cv::RotatedRect, cv::Mat>>>
make_recognizer_model_batches(
    const std::vector<std::pair<cv::RotatedRect, cv::Mat>> &input, int n) {
  ...
  for (int i = 0; i < n; ++i) {
    ...
    std::vector<std::pair<cv::RotatedRect, cv::Mat>> chunk(
...
std::string rotated_rect_to_tesseract_format(const cv::RotatedRect &rect) {
  ...
  float min_x = std::min(std::min(vertices[0].x, vertices[1].x),
                         std::min(vertices[2].x, vertices[3].x));
  float min_y = std::min(std::min(vertices[0].y, vertices[1].y),
                         std::min(vertices[2].y, vertices[3].y));
  float max_x = std::max(std::max(vertices[0].x, vertices[1].x),
                         std::max(vertices[2].x, vertices[3].x));
  float max_y = std::max(std::max(vertices[0].y, vertices[1].y),
                         std::max(vertices[2].y, vertices[3].y));
...
std::string to_json(const std::vector<std::pair<std::string, cv::RotatedRect>>
                        &predicted_text_bbox_pairs) {
  std::stringstream ss;
  ss << "[";
...
int run_ocr(std::string image_path, std::string weights_dir,
  ...
  float canvas_size =
      1024; // This has a big effect on memory and ability to detect boxes
  float mag_ratio = 1.0;

  cv::Mat image_resized;
  float target_ratio;
  cv::Size size_heatmap; // TODO - can probably get rid of this size_heatmap var
  std::tie(image_resized, target_ratio, size_heatmap) =
      resize_aspect_ratio(image, canvas_size, cv::INTER_LINEAR, mag_ratio);

  float ratio_h = 1 / target_ratio;
  float ratio_w = 1 / target_ratio;

  torch::Tensor image_tensor = torch::from_blob(
      image_resized.data, {1, image_resized.rows, image_resized.cols, 3},
      torch::kByte);

  image_tensor = image_tensor.permute(
      {0, 3, 1, 2}); // Rearrange dimensions to {1, 3, 32, 128}
  image_tensor = image_tensor.to(torch::kFloat);
  image_tensor =
      image_tensor.div(255.0); // Normalize pixel values (0-255 -> 0-1)
  ...
  for (int64_t batch_index = 0; batch_index < batch_size; ++batch_index) {
    ...
    float low_text = 0.4;
    auto result = get_detected_boxes(score_text, score_link, text_threshold,
    ...
    cv::imwrite(outputs_dir + "/" + image_file_name + "_detector_crops.jpg",
                all_cropped_images);
    ...
    std::vector<std::pair<std::string, cv::RotatedRect>>
        predicted_text_bbox_pairs;
    std::vector<std::vector<std::pair<cv::RotatedRect, cv::Mat>>> batches =
        make_recognizer_model_batches(text_regions, 4);
    std::size_t n_parseq_batches = batches.size();
    std::size_t parseq_batch_offset = 0;
    for (int64_t parseq_batch_index = 0; parseq_batch_index < n_parseq_batches;
         ++parseq_batch_index) {
      ...
      for (auto &text_region : batches[parseq_batch_index]) {
        ...
        torch::Tensor parseq_image_tensor = torch::from_blob(
            parseq_image_input.data,
            {1, parseq_image_input.rows, parseq_image_input.cols, 3},
            torch::kByte);
        parseq_image_tensor = parseq_image_tensor.permute(
            {0, 3, 1, 2}); // Rearrange dimensions to {1, 3, 32, 128}
        parseq_image_tensor = parseq_image_tensor.to(torch::kFloat);
        parseq_image_tensor = parseq_image_tensor.div(
            255.0); // Normalize pixel values (0-255 -> 0-1)
        parseq_batch.push_back(parseq_image_tensor);
        predicted_text_bbox_pairs.push_back(
            std::make_pair("", text_region.first)); // Set placeholder text
      }
      ...
      Tokenizer tokenizer;
      std::vector<std::string> token_batch =
          tokenizer.decode(parseq_pred, false);
      std::size_t token_batch_size = token_batch.size();
      for (int64_t token_item_index = 0; token_item_index < token_batch_size;
           ++token_item_index) {
        ...
        predicted_text_bbox_pairs[parseq_batch_offset + token_item_index]
    ...
    auto elapsed_time = std::chrono::duration_cast<std::chrono::milliseconds>(
                            end_time - start_time)
                            .count();
    std::cout << "Elapsed time: " << (elapsed_time * 0.001) << " seconds "
              << std::endl;

    std::size_t n_predicted_pairs = predicted_text_bbox_pairs.size();
    for (int64_t pred_pair_index = 0; pred_pair_index < n_predicted_pairs;
         ++pred_pair_index) {
      ...
      const std::string &predicted_text =
          predicted_text_bbox_pairs[pred_pair_index].first;
      cv::Point text_origin(
          corners[0].x, corners[0].y); // You can adjust the position as needed
      int font_face = cv::FONT_HERSHEY_SIMPLEX;
      double font_scale = 0.5;
      int font_thickness = 2;
      cv::Scalar text_color(0, 0, 255); // BGR color: red
      cv::putText(image, predicted_text, text_origin, font_face, font_scale,
    ...
    save_to_file(outputs_dir + "/" + image_file_name + "_results.json",
                 json_str);
  }
